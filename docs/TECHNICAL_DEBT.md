...
| ~~P1~~ ✅ | Native Windows smoke test blocked by missing PostGIS | local PostgreSQL 17 on review machine | PostGIS was missing from the local PostgreSQL 17 install, so the native Windows smoke test could not complete. | Native Windows setup could not be fully verified on this machine. | Resolved by verifying the full stack via Docker Compose (`postgis/postgis:15-3.4` image). Native PostGIS install still needed for non-Docker Windows dev. |
...
| ~~P2~~ ✅ | Per-request external API calls | `backend/app/routers/fields.py` | TTL caching added via `reading_source_status`: weather, NDVI, and forecast are only refreshed after `READING_CACHE_TTL_SECONDS` (default 6 hours). | External service rate limits, slow response times, unnecessary load. | Closed in Phase 2. Background scheduler still optional. |
...
| ~~P2~~ ✅ | `alerts` table does not store delivery status | `database/migrations/006_alerts_delivery_status_and_reading_ttl.sql` | Added `status`, `provider_message_id`, `provider_error` columns; `get_alerts()` now persists the WhatsApp result. | Cannot tell whether an alert was actually delivered; no audit trail. | Closed in Phase 2. |
